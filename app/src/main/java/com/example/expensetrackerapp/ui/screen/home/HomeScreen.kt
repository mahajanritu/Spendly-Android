...
fun Tabs(tabIndex: Int, viewModel: MainViewModel) {
    Row(
        ...
        TabsList.forEachIndexed { index, items ->
...
fun SummaryBox(tabIndex: Int, expenseList: State<List<Expense>>) {
    val expenseListSummary = expenseList.value.sumOf { it.amount }
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .height(120.dp)
...
fun ListSummary(viewModel: MainViewModel, expenseList: State<List<Expense>>) {
    Column {
        // FIXED: Date header text color set to Purple
        Text(
        ...
        if (expenseList.value.isEmpty()) {
            ...
            LazyColumn {
                items(expenseList.value, key = { key -> key.id }) { expense ->
    ...
    Card(
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxHeight()) {
                // FIXED: Icon tint updated to match Purple theme
                Icon(
        ...
        if (showBottomSheetDetail.value) {
            ModalBottomSheet(
                onDismissRequest = { showBottomSheetDetail.value = false },
                containerColor = Color.White
            ) {
                DetailExpenseScreen(
                    showBottomSheet = showBottomSheetDetail,
                    showBottomSheetModify = showBottomSheetModify,
                    viewModel = viewModel,
                    expense = expense
                )
            }
