...
fun BottomBar(mainNavController: NavHostController, navBackStackEntry: NavBackStackEntry?) {
    NavigationBar(
        ...
        itemsNavigation.forEach { items ->
            val isSelected = items.title.lowercase() == navBackStackEntry?.destination?.route
            NavigationBarItem(
                colors = NavigationBarItemDefaults.colors(
                    indicatorColor = Color.White,

                    // FIXED: Selected icon aur text ke liye #7766F7 set kiya gaya hai
                    selectedIconColor = Color(0xFF7766F7),
                    selectedTextColor = Color(0xFF7766F7),
